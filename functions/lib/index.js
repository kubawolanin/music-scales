...
const tonal_1 = require("tonal"); // Interval, Distance, Chord
const tonal_detector_1 = require("tonal-detector");
...
const decodeScale = app => {
    ...
    if (scaleName) {
        const decodedScale = tonal_1.Scale
            .notes(rootNote + ' ' + scaleName)
            .map(scaleNote => normalizeNote(scaleNote))
            .join(' – </s><s>');
        const verbalResponse = sprintf_js_1.sprintf(getRandomValue(strings.scale.decode), {
...
const interval = app => {
    ...
    if (rootNote && targetNote) {
        ...
        const verbalResponse = sprintf_js_1.sprintf(getRandomValue(strings.interval.decode), {
            rootNote: normalizeNote(rootNote),
...
const noteFreq = app => {
    ...
    if (note && octave) {
        ...
        const frequency = tonal_1.Note.freq(noteName).toString();
        const midi = tonal_1.Note.midi(noteName);
        const verbalResponse = sprintf_js_1.sprintf(getRandomValue(strings.note.freq), {
...
const encodeScale = app => {
    ...
    let scaleNames = tonal_detector_1.scale([note1, note2, note3, note4, note5, note6, note7]);
    if (DEBUG_LOGS) {
        console.log('Received data', data);
    ...
    if (scaleNames.length) {
        ...
        const verbalResponse = sprintf_js_1.sprintf(getRandomValue(label), { scaleNames: scaleNames });
        app.tell(app.buildRichResponse()
            .addSimpleResponse(`<speak>${verbalResponse}</speak>`)
        // .addBasicCard(card)
        );
...
const encodeChord = app => {
    ...
    let chords = tonal_detector_1.chord(notes);
    if (DEBUG_LOGS) {
        console.log('Received data', data);
    ...
    if (chords.length) {
        ...
        const verbalResponse = sprintf_js_1.sprintf(getRandomValue(label), { chords: chords });
        app.tell(app.buildRichResponse()
            .addSimpleResponse(`<speak>${verbalResponse}</speak>`)
        // .addBasicCard(card)
        );
...
const scoreImage = functions.https.onRequest((request, response) => {
    ...
    vfSystem.addStave({
        voices: [
            score.voice(score.notes('C4/8, D4, E4, F#4, G#4, A4, B4/q', {
                stem: 'up'
            }))
        ]
